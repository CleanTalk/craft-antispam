...
class API
{
    ...
    public static function methodSecurityLogs($api_key, $data)
    {
        $request = array(
            ...
            'timestamp'   => \Cleantalk\Common\current_time('timestamp'),
            'data'        => json_encode($data),
            'rows'        => count($data),
        );
    ...
    public static function sendRequest($data, $_url = self::URL, $timeout = 10)
    {
        ...
        $data['agent'] = ! empty($data['agent'])
            ? $data['agent']
            : (defined('CLEANTALK_AGENT') ? \Cleantalk\Common\CLEANTALK_AGENT : self::AGENT);

        // Possibility to switch API url
        $url = defined('CLEANTALK_API_URL') ? \Cleantalk\Common\CLEANTALK_API_URL : $_url;
